...
namespace FreeDraw
{
    ...
    public class Drawable : MonoBehaviour
    {
        ...
        internal Texture2D RenderTex(byte color, List<IntPoint> points)
        {
            var tex = new Texture2D(drawable_texture.width, drawable_texture.height);
            tex.SetPixels(clean_colours_array);
            tex.anisoLevel = 0;
            var pic = tex.GetPixels32();
            for (var i = 0; i < points.Count; ++i)
            {
                MarkPixelsToColour(points[i], 3, ColorsTable[color], drawable_texture.width, drawable_texture.height, ref pic);
            }
...
// BRUSH TYPES. Implement your own here
        ...
        public void MarkPixelsToColour(IntPoint center_pixel, int pen_thickness, Color color_of_pen, int width, int height, ref Color32[] pic)
        {
            // Figure out how many pixels we need to colour in each direction (x and y)
        ...
        public void MarkPixelToChange(int x, int y, Color color, int width, Color32[] pic)
        {
        ...
        {
            var tex = new Texture2D(drawable_texture.width, drawable_texture.height);
            tex.SetPixels(drawable_texture.GetPixels());
            tex.anisoLevel = 0;
            tex.Apply();
            return tex;
        }
